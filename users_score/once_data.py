...
class OnceData:
    ...
    def create( self, k ):
        ...
        for kk in self.race_data[k].keys():
            ...
            score = self.us.get_score()
            key_score = str( int( score / 5 ) )
            lib.dic_append( self.data, year, {} )
            lib.dic_append( self.data[year], key_score, { "recovery": 0, "count": 0 } )
            self.data[year][key_score]["count"] += 1
            self.data[year][key_score]["recovery"] += odds
